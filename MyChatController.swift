...
class MyChatMessage : NSObject {
    
    enum SentBy : String {
        case User = "MyChatMessageSentByUser"
        case Opponent = "MyChatMessageSentByOpponent"
    }
    ...
    */
    ...
    class func SentByUserString() -> String {
        return MyChatMessage.SentBy.User.rawValue
    }
    
    class func SentByOpponentString() -> String {
        return MyChatMessage.SentBy.Opponent.rawValue
    }
    ...
    var sentByString: String {
        ...
        set {
            if let sentBy = SentBy(rawValue: newValue) {
                ...
                println("MyChatMessage.FatalError : Property Set : Incompatible string set to SentByString!")
            }
        }
    }
    ...
    convenience init (content: String, sentByString: String) {
        if let sentBy = SentBy(rawValue: sentByString) {
            ...
            fatalError("MyChatMessage.FatalError : Initialization : Incompatible string set to SentByString!")
        }
    }
...
class MyChatMessageCell : UITableViewCell {
    ...
    */
    func setupWithMessage(message: MyChatMessage) -> CGSize {
    ...
    private func styleTextViewForSentBy(sentBy: MyChatMessage.SentBy) {
...
@objc protocol MyChatControllerDelegate {
    
    optional func shouldChatController(chatController: MyChatController, addMessage message: MyChatMessage) -> Bool
    optional func chatController(chatController: MyChatController, didAddNewMessage message: MyChatMessage)
}

class MyChatController : UIViewController, UITableViewDelegate, UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate, ChatInputDelegate {
    
    // MARK: Constants
    
    private struct Constants {
        static let MessagesSection: Int = 0;
        static let MessageCellIdentifier: String = "MyChatController.Constants.MessageCellIdentifier"
    }
    ...
    */
    var messages: [MyChatMessage] = []
    var opponentImage: UIImage?
    weak var delegate: MyChatControllerDelegate?
    var hasImageView: Bool = false
    var dirFilesArray: NSMutableArray = NSMutableArray()
    
    // MARK: Private Properties
    
    private let sizingCell = MyChatMessageCell()
    ...
    override func viewDidLoad() {
        ...
        button.addTarget(self, action: Selector("actioncall:"), forControlEvents:UIControlEvents.TouchUpInside)
    ...
    private func setupTableView() {
        ...
        tableView.registerClass(MyChatMessageCell.classForCoder(), forCellReuseIdentifier: "identifier")
    ...
    func addNewMessage(message: MyChatMessage) {
        messages += [message]
    ...
    func chatInput(chatInput: ChatInput, didSendMessage message: String) {
        let newMessage = MyChatMessage(content: message, sentBy: .User)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        var mycell = tableView.dequeueReusableCellWithIdentifier("identifier", forIndexPath: indexPath) as! MyChatMessageCell
        
        let message = self.messages[indexPath.row]
        
        mycell.opponentImageView.image = message.sentBy == .Opponent ? self.opponentImage : nil
        
        
        mycell.setupWithMessage(message)
